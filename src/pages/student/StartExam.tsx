...
type AnswerMap = Record<string, any>; // questionId -> response
...
export default function StartExam() {
  ...
  // 1) Load exam + restore draft + start attempt if needed
  useEffect(() => {
    (async () => {
      if (!examId) return;
      setLoading(true);
      ...
      try {
        // Load exam (questions without answers)
  ...
  // 3) Autosave answers (debounced-ish)
  useEffect(() => {
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={`Answered ${progress.answered}/${progress.total} • Attempt: ${attemptId?.slice(0, 8) || "-"}`}
        right={
          ...
          <div className="card shadow-sm">
            <div className="card-body">
              ...
              >
                {questions.map((q, idx) => {
                  ...
                  return (
                    <button
                      key={q.id}
                      className={`btn text-start btn-sm ${isActive ? "btn-primary" : "btn-outline-secondary"}`}
              ...
              <div className="mt-3 text-muted small">
                Tip: Your answers autosave. You can refresh without losing
                progress.
              </div>
            </div>
          </div>
...
function MCQBlock({
  ...
  // Support options as object {A:"",B:""} or array ["..",".."]
